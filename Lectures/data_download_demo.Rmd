...
* The R functions and packages we use help a lot in avoiding manual typing out of things, but the `readxl` package is still in relatively early development. It shows some limitations here in handling certain aspects of these spreadsheets that force me to seek inelegant solutions.

In this case, I *could* have done this all more quickly by downloading each of the files individually, opening them up one-by-one in Excel, creating a "master" Excel file, manually pasting in the data from each one of these, fixing up column type issues within Excel, exporting this to a CSV, and importing the master CSV into R. However, this is an unappealing solution:

* It does not provide documentation of my process. You'd have to take my word for what I said I did, and maybe I am a liar, very sloppy, or not forthcoming about my process.

* It provides many opportunities for additional human error (like the Reinhart and Rogoff failure to drag the formula all the way down). For example, maybe I thought I copied data for 2009, and tried to paste it at the bottom of the master spreadsheet. However, the copying didn't work, and instead I end up pasting 2008's data twice without noticing because it looks so similar.

* I would need to be clever about running manual checks on the data to detect issues like the above, whereas R is happy to let me know about any problems.
...
What would make sense here is to use a bit of looping to generate these URLs without a ton of copying and pasting. Then when I download these spreadsheets, I'll rename the files something better and consistent for posterity: `CA_OSHPD_utilization_[YEAR].xls`, inside a folder called `Downloaded_data`. I'll then pre-allocate a list to store each of the files when I read them into R, and then I can use `bind_rows` in `dplyr` to combine them from being annual data sets inside a list to one big data frame. Then, I can remove the list to free up memory, do any further tidying on the whole file, and save it in an R format for later use.
...
# This path goes one level up (the ..) and then into a folder
# called Downloaded_data. You will need to create or modify
...
Looks good so far -- now for the harder part. Let's make a vector of the URLs to download from. We'll try to avoid manual repetition by making a variable that stores the common part of the URL, and a variable that captures the stuff that does change, and `paste0`ing it all together:
...
Doesn't look like anything is wrong there length-wise, though the names don't match those from a previous file -- why?
...
It looks like the first year of data had underscores in the names, while this weird 2013 file had spaces instead. That *shouldn't* be related to the problems I'm having, but does mean I should manually specify the column names as well as types when reading the data in to get consistency.
...
Googling this new error message, I encounter [a suggestion here](https://github.com/hadley/readxl/issues/81) that there may be secret blank columns in the Excel file. What this usually means is there was some data in those columns at some point in this spreadsheet's past, and they were blanked out but not deleted, so Excel thinks they sort of have something in them --- but they don't really, and now R is thinking there's something there. Confusing? Very.

I open this data in Excel, delete a bunch of blank columns to the right manually (i.e. select the first blank column, press Cntl-Shift-Right Arrow to select everything to the right, and then choose Delete from a menu). Then, I save a new version of the file where I delete a bunch of blank columns to the right of the data range. I'm giving it a different name than I had originally saved it: I want my analysis to be as reproducible as possible, which philosophically includes never, ever messing with the original data files, even I have to do a kludgey non-programmable fix for this particular nuisance file.

```{r reimport_2013_no_blanks, cache=TRUE, error=TRUE}
no_blanks_2013 <- read_excel("../Downloaded_data/CA_OSHPD_utilization_2013_blanks_deleted.xls",
                             col_types = OSHPD_col_types)
```

## Whack-a-mole in 2014 and 2015

Victory! Now I'll change my vector of filenames to have the new path for 2013, make a new vector for the column names based on the first file that worked from the previous run of the loop, and then re-run it, this time manually specifying column names and skipping the header row with `skip = 1`. Brace yourself for several hundred lines of errors to scroll past:

```{r read_in_data_3, cache=TRUE, error=TRUE}
OSHPD_col_names <- colnames(yearly_frames[[1]])

new_filenames[i] <- "../Downloaded_data/CA_OSHPD_utilization_2013_blanks_deleted.xls"
...
First, fixing the 2014 issue: When I open that file in Excel, I see some ding-dong has stored this column as text instead of a number (there are little green warning flags about this). I'll make an exception for this file and read column 18 in as a character, and then convert it to numeric using `mutate_each_` in `dplyr` (preferring not to mess with the original Excel files if I don't absolutely have to).

```{r read_in_data_4, cache=TRUE, error=TRUE}
OSHPD_col_types_2014 <- OSHPD_col_types
OSHPD_col_types_2014[18] <- "text"

for(i in seq_along(new_filenames)) {
    if(file.exists(new_filenames[i])) {
        print(paste0("Now trying ", years[i]))
        # special column types for 2014 only
        if(years[i] == 2014) {
            ctypes <- OSHPD_col_types_2014
        } else {
...
The phone number I can deal with. My bigger problem is someone thought it would be nice to provide undocumented extra columns for 2015, what a treat! So what are they compared with the columns in the other files? I'll make a data frame comparing them, padding out the extra spaces with missing values for the other files:
...
After much effort, we have read everything in. Let's combine the files for all years using `bind_rows` from `dplyr`. I'm going to use the `.id` argument to add a column called `orig_file` for which file number each row came from for further debugging.
...
Some from file 12 (`r years[12]`), some from file 16 (`r years[16]`). I'm happy I made that file source column to tell me this! Let's look at those files in the list and see if there are extra rows read in at the end:
...
Indeed, there were some fake-blank rows missing basically all info at the end of these files. If I look at the original Excel files, these were fake-blank rows (like the fake-blank columns that caused me grief with 2013). Fake-blank rows are no sweat, though, compared with the fake-blank columns breaking `read_excel`. I'll just filter out rows with `NA` values from a column that should be fully populated.
...
Something I'm observing looking at the file is that some of the values in the columns that should be character but look like numbers (`FAC_NO`, `HSA`, `HFPA`) have weird decimals added:
...
These are going to be annoying if I wanted to group by any of these variables, because I need identifiers to be consistent. I want to take anything from a period onwards in these columns and remove it. We'll see more about this in the string-cleaning lecture, but I'll use the `gsub` function in base R inside `mutate_each` to do this.

```{r fix_character_cols}
CA_OSHPD_util <- CA_OSHPD_util %>%
    # translating the regular expression:
    # \\. matches the period
    # 0+ matches at least one zero and possibly more
    # replace these with nothing (empty string)
...
Much better! There are probably more issues at this point, but not ones I'm noticing --- not at least until I decide to do some more data analysis or visualization. This is good enough for now.

# Exporting the file and memory cleanup

Now, I'll save this formatted R file so that I can use it for whatever I want.
