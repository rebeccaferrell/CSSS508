...
You will be walked through these steps. For each code chunk, fill in code and change it from `eval=FALSE` to `eval=TRUE` when you have it working.
...
uw_calculus_gpa <- 0.3 + 0.7 * hs_math_gpa + 0.3 * hs_calculus + 0.1 * uw_precalc + rnorm(n, mean = 0, sd = 0.5)
...
We get estimated variances for linear regression coefficients using the following formula:

$$\widehat{\text{Var}}(\hat{\mathbf{\beta}}) = \hat{\sigma}^2  \left(\mathbf{X}^T \cdot \mathbf{X} \right)^{-1}$$

We then take the square root of the diagonal of $\widehat{\text{Var}}(\hat{\mathbf{\beta}})$ to get the standard errors for each coefficient. (The off-diagonal terms are covariances.)
...
Now let's calculate the residual variance $\hat{\sigma}^2$. This has $n-p-1$ in the denominator, so let's compute $p$ while we're at it. You already *know* $p=3$, but instead of hard-coding this, use a function to compute it as the number of columns of the design matrix minus 1. That way, if had you added or deleted a column, the math would still be correct. Additionally, after you've calculated the residual variance, I've put `as.numeric()` at the end to convert it to a single scalar number instead of a 1 by 1 matrix.
...
Next, find the variance matrix $\widehat{\text{Var}}(\hat{\mathbf{\beta}})$ using $A$ and $\hat{\sigma}^2$. (We needed the `as.numeric()` step above because otherwise we would be multiplying together two matrices whose dimensions can't be multiplied.)

```{r beta_variance, eval=FALSE}
# FILL THIS IN AND DELETE THIS COMMENT
beta_var <-
```

Finally, we go from variance matrix $\widehat{\text{Var}}(\hat{\mathbf{\beta}})$ to standard errors by taking the square root of its diagonal.
...
Now, let's make a matrix comparing the fitted parameters. Make the second column of the matrix have name "manual" and the rows be the values of $\hat{\mathbf{\beta}}$ you computed. Make the second column of the matrix have name "lm" and the rows be the values of the fitted parameters using `lm()` from before. Use the `pander` package to display a nice formatted table. If everything went right, these values should match!
...
Now do the same thing for standard errors of the parameter estimates.
...
Last, compare the values of the residual variance. The residual *standard deviation* in the `lm()` version can be found using the `summary()` list in an entry called `sigma`. You can convert this to residual variance by squaring it.
...
Congratulations, you've replicated R's linear regression function!